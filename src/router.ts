  ...
  generate(name: string, opt_params?: RouteParams, absolute?: boolean): string {
    ...
    route.tokens.forEach((token) => {
      ...
      if (token[0] === 'variable') {
        ...
        const hasDefault =
          route.defaults &&
          !Array.isArray(route.defaults) &&
          typeof token[3] === 'string' &&
    ...
    if (route.requirements && '_scheme' in route.requirements && this.getScheme() != route.requirements._scheme) {
      ...
      url =
        `${route.requirements._scheme 
        }://${ 
        currentHost 
        }${currentHost.indexOf(`:${  port}`) > -1 || port === '' ? '' : `:${  port}` 
        }${url}`;
    } else if (
      typeof route.schemes !== 'undefined' &&
      typeof route.schemes[0] !== 'undefined' &&
      ...
      url =
        ...
        this.getHost() 
        }${this.getHost().indexOf(`:${  port}`) > -1 || port === '' ? '' : `:${  port}` 
        }${url}`;
    }
